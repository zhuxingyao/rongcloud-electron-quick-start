(function (win, dependencies, components) {
  ...
  function watchUpdatedExpansion(updatedExpansion) {
    var time = utils.timestampToString(updatedExpansion.updatedTime),
      expansion = JSON.stringify(updatedExpansion.expansion),
      msgUId = updatedExpansion.messageUId;
    vueInstance.addOutput('消息 KV 更新', updatedExpansion, 0, [], {
      color: utils.TypeColor.MSG
    });
    innerEl(`${time}: 监听更新, msgUId: ${msgUId} | ${expansion}`)
  }
  function watchDeletedExpansion(deletedExpansion) {
    ...
    vueInstance.addOutput('消息 KV 删除', deletedExpansion, 0, [], {
      color: utils.TypeColor.MSG
    });
    innerEl(`${time}: 监听删除, keys: ${keys} | msgUId: ${msgUId}`)
  }

  function watchTagChange(status) {
    console.log('标签变更', status)
    vueInstance.addOutput('标签变更', status, 0, [], {
      color: utils.TypeColor.MSG
    });
  }

  function watchFinished(status) {
    vueInstance.addOutput('离线数据加载完毕', status, 0, [], {
      color: utils.TypeColor.MSG
    });
  }

  function watchConversationTagChange(status) {
    console.log('会话标签变更', status)
    vueInstance.addOutput('会话标签变更', status, 0, [], {
      color: utils.TypeColor.MSG
    });
  }
  function watchTypingChange(status) {
    console.log('typing状态变更', status)
    vueInstance.addOutput('typing状态变更', status, 0, [], {
      color: utils.TypeColor.MSG
    });
  }
  ...
  function login(config) {
    ...
    return Service.init(config, {
      ...
      watchUpdatedExpansion,
      watchDeletedExpansion,
      watchTagChange,
      watchConversationTagChange,
      watchTypingChange,
      watchFinished
